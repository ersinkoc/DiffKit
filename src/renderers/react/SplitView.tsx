/**
 * Split View component
 */

import type { CSSProperties } from 'react';
import { useMemo } from 'react';
import type { SplitViewProps } from './types.js';
import type { Change } from '../../core/types.js';
import { useDiff } from './hooks/useDiff.js';
import { useThemeCSSVars } from './hooks/useTheme.js';
import { LineNumber } from './LineNumber.js';

interface LinePair {
  left: Change | null;
  right: Change | null;
}

/**
 * Split diff view component
 * Shows old and new content side by side
 */
export function SplitView({
  old: oldContent,
  new: newContent,
  lineNumbers = true,
  wrapLines = false,
  showHeader = true,
  algorithm = 'myers',
  granularity = 'line',
  context = 3,
  theme = 'github-dark',
  onLineClick,
  onHunkClick,
  className = '',
  style,
}: SplitViewProps) {
  const { result, loading, error } = useDiff(oldContent, newContent, {
    algorithm,
    granularity,
    context,
  });

  const cssVars = useThemeCSSVars(theme);

  const containerStyle = useMemo(() => {
    return {
      ...cssVars,
      ...style,
    } as CSSProperties;
  }, [cssVars, style]);

  const wrapClass = wrapLines ? 'diffkit-wrap' : '';
  const baseClass = `diffkit-diff diffkit-split ${wrapClass}`;
  const fullClass = className ? `${baseClass} ${className}` : baseClass;

  if (loading) {
    return (
      <div className={fullClass} style={containerStyle}>
        <div className="diffkit-loading">Computing diff...</div>
      </div>
    );
  }

  if (error) {
    return (
      <div className={fullClass} style={containerStyle}>
        <div className="diffkit-error">Error: {error.message}</div>
      </div>
    );
  }

  if (!result || result.hunks.length === 0) {
    return (
      <div className={fullClass} style={containerStyle}>
        <div className="diffkit-empty">No differences found</div>
      </div>
    );
  }

  return (
    <div className={fullClass} style={containerStyle}>
      {result.hunks.map((hunk, hunkIndex) => {
        const pairs = buildLinePairs(hunk.changes);

        return (
          <div
            key={hunkIndex}
            className="diffkit-hunk"
            onClick={onHunkClick ? () => onHunkClick(hunk) : undefined}
          >
            {showHeader && <div className="diffkit-hunk-header">{hunk.header}</div>}
            <div className="diffkit-hunk-content">
              {pairs.map((pair, pairIndex) => (
                <div key={pairIndex} className="diffkit-row">
                  <SplitSide
                    change={pair.left}
                    side="left"
                    lineNumbers={lineNumbers}
                    onLineClick={onLineClick}
                  />
                  <SplitSide
                    change={pair.right}
                    side="right"
                    lineNumbers={lineNumbers}
                    onLineClick={onLineClick}
                  />
                </div>
              ))}
            </div>
          </div>
        );
      })}
    </div>
  );
}

/**
 * Build pairs of lines for split view
 */
function buildLinePairs(changes: Change[]): LinePair[] {
  const pairs: LinePair[] = [];
  const deletions: Change[] = [];
  const additions: Change[] = [];

  for (const change of changes) {
    if (change.type === 'normal') {
      flushPairs(pairs, deletions, additions);
      pairs.push({ left: change, right: change });
    } else if (change.type === 'delete') {
      deletions.push(change);
    } else if (change.type === 'add') {
      additions.push(change);
    }
  }

  flushPairs(pairs, deletions, additions);
  return pairs;
}

function flushPairs(pairs: LinePair[], deletions: Change[], additions: Change[]): void {
  const maxLen = Math.max(deletions.length, additions.length);

  for (let i = 0; i < maxLen; i++) {
    pairs.push({
      left: deletions[i] ?? null,
      right: additions[i] ?? null,
    });
  }

  deletions.length = 0;
  additions.length = 0;
}

interface SplitSideProps {
  change: Change | null;
  side: 'left' | 'right';
  lineNumbers: boolean;
  onLineClick?: (lineNumber: number, side: 'old' | 'new') => void;
}

function SplitSide({ change, side, lineNumbers, onLineClick }: SplitSideProps) {
  const isEmpty = !change;
  const typeClass = isEmpty
    ? 'diffkit-empty'
    : change.type === 'add'
      ? 'diffkit-add'
      : change.type === 'delete'
        ? 'diffkit-delete'
        : 'diffkit-normal';

  const sideClass = `diffkit-side diffkit-${side} ${typeClass}`;

  const handleClick = () => {
    if (!change || !onLineClick) return;
    const lineNum = side === 'left' ? change.oldLineNumber : change.newLineNumber;
    if (lineNum) {
      onLineClick(lineNum, side === 'left' ? 'old' : 'new');
    }
  };

  return (
    <div className={sideClass} onClick={handleClick}>
      {lineNumbers && (
        <div className="diffkit-gutter">
          <LineNumber
            number={change ? (side === 'left' ? change.oldLineNumber : change.newLineNumber) : undefined}
            side={side === 'left' ? 'old' : 'new'}
          />
        </div>
      )}
      <div className="diffkit-content">{change ? change.content : ''}</div>
    </div>
  );
}

export default SplitView;
